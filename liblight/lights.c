...
write_rgb_str(LedIdentifierRGB rgb_leds, char *value)
{
    ...
    if (fd.red >= 0 && fd.green >= 0 && fd.blue >= 0) {
        ...
        char buffernull[PAGE_SIZE];
        int bytes = sprintf(buffer, "%s\n", value);
        int nullbytes = sprintf(buffernull, "0,0\n");
        LedIdentifier amt;

        if (rgb_leds.red) {
            amt.red = write(fd.red, buffer, bytes);
        } else {
            amt.red = write(fd.red, buffernull, nullbytes);
        }
        if (rgb_leds.green) {
            amt.green = write(fd.green, buffer, bytes);
        } else {
            amt.green = write(fd.green, buffernull, nullbytes);
        }
        if (rgb_leds.blue) {
            amt.blue = write(fd.blue, buffer, bytes);
        } else {
            amt.blue = write(fd.blue, buffernull, nullbytes);
        }
        close(fd.red);
        close(fd.green);
        close(fd.blue);
